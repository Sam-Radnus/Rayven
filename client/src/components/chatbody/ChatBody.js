...
const ChatBody = ({ match, currentChattingMember, setOnlineUserList }) => {
  ...
  const [position, setPosition] = useState({ x: -150, y: 37 });
  ...
  const [textareaSize, setTextareaSize] = useState({ width: 1200, height: 50 });
  
  const initialHeight = 200; // Initial height of the textarea
  ...
  const handleDragStop = () => {
    setHeight((prevHeight) => prevHeight - dragDeltaY);
    setDragDeltaY(0);
  };
